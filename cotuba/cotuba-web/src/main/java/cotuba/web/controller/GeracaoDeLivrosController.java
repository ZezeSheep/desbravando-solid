...
public class GeracaoDeLivrosController{
    ...
    public byte[] geraPDF(@PathVariable("id") Long id, HttpServletResponse response) throws IOException{
        return ebookParaOFromato(FormatoEbook.PDF, response, id);
    ...
    public byte[] geraEPUB(@PathVariable("id") Long id, HttpServletResponse response) throws IOException{
        return ebookParaOFromato(FormatoEbook.EPUB, response, id);
    }

    private byte[] ebookParaOFromato(FormatoEbook formato, 
        HttpServletResponse response, Long idLivro) throws IOException{
        response.addHeader(HttpHeaders.CONTENT_DISPOSITION, 
            "attachment;filename=book."+formato.name().toLowerCase());

        Livro livro = repositorioDeLivros.findById(idLivro).orElseThrow(
